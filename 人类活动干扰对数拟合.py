...
variant = variant[:, 3].reshape(-1)
shap_value = shap_value[:, 3].reshape(-1)
...
# 使用对数函数模型进行拟合
popt, pcov = curve_fit(logarithmic_func, x_train, y_train)
a, b = popt

# 打印拟合对数方程
equation = f'{a:.3f} * log(x) + {b:.3f}'
print('Logarithmic Regression:')
print(f'Equation: {equation}')

# 计算拟合精度和误差指标
y_test_pred = logarithmic_func(x_test, a, b)
...
plt.scatter(x_display, y_display, color='blue',  alpha=0.5)
plt.plot(x_fit, y_fit, color='green', )

# 添加拟合函数和R2值的文本注释
annotation = f'{equation}\n $R^2$ : {test_score:.3f}'
plt.text(0.5, 0.05, annotation, transform=plt.gca().transAxes, fontsize=12)

plt.xlabel('kaikuang')
plt.ylabel('shap_value')
